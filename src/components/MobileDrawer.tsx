...
const ALL_NAV_ITEMS: NavItem[] = [
  { id: 'superadmin', label: 'Super Admin HQ', icon: ShieldCheck, badge: 'Master', badgeColor: 'bg-amber-500/10 text-amber-400 border-amber-500/20' },
  { id: 'dashboard', label: 'Executive AI', icon: LayoutDashboard },
  { id: 'pos', label: 'Billing & POS', icon: ShoppingCart, badge: 'Fast', badgeColor: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' },
  { id: 'documents', label: 'Documents & GST', icon: FileText },
      ...
      <div className={`relative w-4/5 max-w-xs h-full flex flex-col justify-between p-5 overflow-y-auto shadow-2xl z-10 transition-transform ${
        ...
        <div className="space-y-5">
                ...
                <div className={`mt-1 p-1 rounded-xl border space-y-1 ${
                  ...
                  {organizations.map(org => (
                    <button
                      ...
                      className="w-full text-left p-2 rounded-lg text-[11px] flex items-center justify-between hover:bg-blue-500/10"
                    >
                      <span className="truncate">{org.name}</span>
                      {org.id === currentOrg.id && <Check className="w-3 h-3 text-blue-500" />}
                    </button>
                  ))}
                </div>
              )}
            ...
            <div className="relative">
              ...
              >
                <div className="flex items-center gap-2 truncate">
                  ...
                  <span className="truncate font-semibold">{activeRole}</span>
                </div>
              ...
              {showRoleSelect && (
                <div className={`mt-1 p-1 max-h-40 overflow-y-auto rounded-xl border space-y-1 ${
                  ...
                  {ROLES.map(role => (
                    ...
                    >
                      <span>{role}</span>
                      {role === activeRole && <Check className="w-3 h-3 text-blue-400" />}
                    </button>
                  ))}
                </div>
              )}
          ...
          <div className="space-y-1 pt-2 border-t border-neutral-800">
            ...
            {ALL_NAV_ITEMS.filter(item => activeRole === 'Super Admin' || item.id !== 'superadmin').map((item) => {
